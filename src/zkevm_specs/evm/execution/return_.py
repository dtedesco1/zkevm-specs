...
def return_(instruction: Instruction):
    # print(f'+ gas_left: {instruction.curr.gas_left}')
    ...
    if instruction.curr.is_create:
        ...
        instruction.rw_counter_offset += copy_rwc_inc
    ...
    if not instruction.curr.is_root and not instruction.curr.is_create:
        ...
        caller_return_offset = instruction.call_context_lookup(CallContextFieldTag.ReturnDataOffset)  # rwc += 1
        caller_return_length = instruction.call_context_lookup(CallContextFieldTag.ReturnDataLength)  # rwc += 1
    ...
    # print(f'+ memory_expansion_gas: {memory_expansion_gas}')

    if instruction.curr.is_root:
        # When a transaction ends with STOP, this call must be persistent
        is_persistent = instruction.call_context_lookup(CallContextFieldTag.IsPersistent)  # rwc += 1
